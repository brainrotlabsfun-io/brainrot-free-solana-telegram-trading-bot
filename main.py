"""
BRAINROTONCHAIN_BOT — Main Entry Point
=======================================
...
from bot.handlers import supreme_access  # SUPREME burn activation
from bot.handlers import profile         # Profile + badges
from bot.handlers import auto_exit       # Supreme Black Auto-Exit Manager
...
from bot.handlers import sol_payment     # sp:* SOL-payment timed Supreme Black access
from bot.handlers import tutorial        # tut:* User Tutorial
from bot.handlers import affiliate       # af:*  Alpha Network affiliate system

# ── Services ───────────────────────────────────────────────────────────────────
from services.pumpportal_ws_service import pump_service
from services.badge_service import init_default_badges
...
import services.sol_payment_worker as sol_payment_worker
import services.watch_alert_worker as watch_alert_worker
import services.affiliate_payment_worker as affiliate_payment_worker
...
async def main() -> None:
    ...
    await seed_system_presets()   # Supreme Black exit presets
    ...
    dp.include_router(supreme_access.router) # supreme:* callbacks + FSM
    dp.include_router(profile.router)        # profile:* callbacks
    dp.include_router(auto_exit.router)      # ae:*      callbacks + FSM (Supreme Black)
    ...
    dp.include_router(affiliate.router)      # af:*      Alpha Network affiliate system  ← before sniper catch-all
    dp.include_router(sol_payment.router)    # sp:*      SOL-payment timed Supreme Black
    ...
    logger.info("=" * 54)
    logger.info("  BRAINROTONCHAIN_BOT v4.0 — Burn & Badge System")
    ...
    sp_task = asyncio.create_task(sol_payment_worker.start(bot))
    wa_task  = asyncio.create_task(watch_alert_worker.start(bot))
    aff_task = asyncio.create_task(affiliate_payment_worker.start(bot))
    ...
    try:
        ...
        sol_payment_worker.stop()
        watch_alert_worker.stop()
        affiliate_payment_worker.stop()
        ws_task.cancel()
        ab_task.cancel()
        ae_task.cancel()
        ct_task.cancel()
